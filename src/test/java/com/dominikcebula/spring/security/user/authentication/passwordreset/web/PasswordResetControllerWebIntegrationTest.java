...
@ActiveProfiles("test")
class PasswordResetControllerWebIntegrationTest {
    @Autowired
    private WebClientFactory webClientFactory;

    @Test
    void shouldDisplayDefaultPasswordResetPage() throws IOException {
        try (WebClient webClient = webClientFactory.getWebClient()) {
            HtmlPage page = webClient.getPage(webClientFactory.getLocalUrl("/passwordReset"));
